...
public struct OrderedDictionary<Key, Value> where Key: Hashable {
	...
	public private(set) var unorderedHash: [Key: Value]
	...
	public func contains(where predicate: (Key) throws -> Bool) rethrows -> Bool {
		try keys.contains(where: predicate)
	}
...
public extension OrderedDictionary {
	...
	struct Iterator: Sequence, IteratorProtocol {
		...
		fileprivate init(_ dictionary: OrderedDictionary) {
			...
			idxReciprocal = dictionary.count
		}
...
extension OrderedDictionary: Encodable where Key: Encodable, Value: Encodable {
	...
	public func encode(to encoder: Encoder) throws {
		...
		if let encodableDictionary = self as? StringRawKeyEncodable {
			let kvPairs = zip(encodableDictionary.orderedStringKeys, values)
...
internal func encodeStringDict<Value: Encodable>(
	...
	var container = encoder.container(keyedBy: StringKey<String>.self)
...
extension OrderedDictionary: Decodable where Key: Decodable, Value: Decodable {
	...
	internal static func decodeStringDict(
		...
		let container = try decoder.container(keyedBy: StringKey<String>.self)
