...
public final class TypeRevisionEncoder: Encoder {

	var path: [TypePath]
    public var codingPath: [CodingKey] { path.map(\.key) }
    ...
    public init(
		...
		userInfo = [:]
		result = TypeInfo(type: Any.self, isOptional: false, container: .single(.null))
	}
	...
	@discardableResult
    public func encode(_ value: Encodable?, type: Encodable.Type) throws -> TypeInfo {
		if !path.isEmpty, path.dropLast().contains(where: { path in path.type == type }) {
            ...
            case let data as Data:
                result.container = .single(.string(data.base64EncodedString()))
                result.type = type
            case let decimal as Decimal:
                result.container = .single(.string(decimal.description))
                result.type = type
            default:
	...
	mutating func nestedUnkeyedContainer() -> UnkeyedEncodingContainer {
		...
		return TypeRevisionSingleValueEncodingContainer(
			...
			result: Ref { [$result] in
				$result.wrappedValue.container.unkeyed ?? .any
